...
def sent_embed_dr_cl(documents,path):
    ...
    sent_embeddings = Sentence_Encoder(embedding_model).embed_documents(documents, verbose=True)    
    ...
    joblib.dump(hdbscan_model, f"{path}/hdbscan_model.sav")
    joblib.dump(umap_model, f"{path}/umap_model.sav")   
...
def main():
    ...
    def doc_tf_idf(documents, doc_length, ngram_range=(1, 2)):
        """
        topic based TF-IDF to create dense clusters allowing for easily interpretable topics
        """
        count = CountVectorizer(ngram_range=ngram_range, stop_words=stop_words).fit(documents)
        terms = count.transform(documents).toarray()
        words = terms.sum(axis=1)
        tf = np.divide(terms.T, words)
        sum_terms = terms.sum(axis=0)
        idf = np.log(np.divide(doc_length, sum_terms)).reshape(-1, 1)
