...
def get_corpus(df, col):
    ...
    df = remove_hyperlinks_htmltags(df, col)
...
class Sentence_Encoder:
    ...
    def embed_documents(self, document, verbose = False):
        ...
        """
        embeddings = self.embedding_model.encode(document, show_progress_bar=verbose).to(device)
        return embeddings
